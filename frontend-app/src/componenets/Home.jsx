          ...
          <Slider {...settings}>
            {courses.map((course) => (
              <div key={course._id} className='p-4'>
                <div className='relative flex-shrink-0 w-55 transition-transform duration-300 transform hover:scale-105'>
                  <div className='bg-gray-900 rounded-lg overflow-hidden'>
                    <img className='h-25 w-full object-contain' src={course.image?.url} alt="" />
                    <div className='p-4 text-center'>
                      <h2 className='text-xl text-white'>{course.description}</h2>
                      <Link
                      to={"/courses"}
                       className='mt-5 bg-orange-500 text-white py-1 px-4 rounded-full hover:bg-blue-400'
